...
@push('styles')
    <style>
        ...
        .hv-g1 {
            background: linear-gradient(135deg, #F5C099, #E07030);
        }

        .hv-g2 {
            background: linear-gradient(135deg, #8AAB7A, #5A8050);
        }

        .hv-g3 {
            background: linear-gradient(135deg, #F0C840, #D09010);
        }

        .hv-g4 {
            background: linear-gradient(135deg, #E87060, #B04030);
        }
        ...
        .resep-card:hover {
            ...
            box-shadow: 0 12px 32px rgba(61, 32, 16, .12);
        }
    ...
    <div class="hero">
        <div class="hero-inner">
            <div>
                ...
                <p class="hero-desc">Temukan ribuan resep lezat dari dapur nusantara. Login untuk menyukai, menyimpan
                    favorit, dan berbagi resep andalanmu!</p>
                <div class="hero-btns">
            ...
            <div class="hero-visual">
                ...
                @foreach ([0, 1] as $col)
                    @php
                        $r1 = $heroReseps->get($col * 2);
                        $r2 = $heroReseps->get($col * 2 + 1);
                    @endphp
                    <div style="display:flex;flex-direction:column;gap:12px;">
                        <div class="hv-card hv-tall {{ $r1 ? '' : $heroGrad[$col * 2] }}">
                            ...
                            @else<span
                                    style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:40px;">{{ $heroFallback[$col * 2] }}</span>
                            @endif
                        ...
                        <div class="hv-card hv-short {{ $r2 ? '' : $heroGrad[$col * 2 + 1] }}">
                            ...
                            @else<span
                                    style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:32px;">{{ $heroFallback[$col * 2 + 1] }}</span>
                            @endif
            ...
            <div class="search-field">
                ...
                <div class="search-input-wrap">
                    ...
                    <input type="text" name="search" class="search-input"
                        placeholder="Cari resep, bahan, atau masakan..." value="{{ request('search') }}">
                </div>
            </div>
        ...
        <div class="section-head">
            <div>
                ...
                <p>
                    ...
                    @else
                        Halaman {{ $reseps->currentPage() }} dari {{ $reseps->lastPage() }} — total
                        {{ $reseps->total() }} resep
                    @endif
                </p>
            </div>
                ...
                @foreach ($reseps as $resep)
                    <a href="{{ route('resep.show', $resep->id) }}" class="resep-card">
                        <div class="resep-img-wrap">
                            @if ($resep->gambar)
                                <img class="resep-img" src="{{ asset('storage/' . $resep->gambar) }}"
                                    alt="{{ $resep->judul }}" loading="lazy">
                            @else
                                <div class="resep-img-fallback">
                                    @php
                                        $emo = ['🍛', '🥗', '🍜', '🍮', '🍳', '🥘', '🍲', '🥗', '🍱', '🧆'];
                                        echo $emo[$loop->index % count($emo)];
                                    @endphp
                                </div>
                            @endif
                        ...
                        <div class="resep-body">
                            ...
                            <div class="resep-footer">
                                ...
                                <div class="resep-author-wrap">
                                    <div class="resep-author-avatar">{{ strtoupper(substr($resep->user->name, 0, 2)) }}
                                    </div>
